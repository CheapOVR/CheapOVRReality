    ...
    public class ReCenterModule : MonoBehaviour
    {
        ...
        void Update()
        {
            if (mainTrackingModule.IsListActive)
            {
                ...
                if (Vector3.Distance(IndexFingerPoint, ThumbFingerPoint) <= 150 && Vector3.Distance(IndexFingerPoint, mainTrackingModule._screen.GetComponent<RectTransform>().rect.GetPoint(mainTrackingModule._currentHandLandmarkLists[0].Landmark[0])) >= Accurancy*100+50)
                {
