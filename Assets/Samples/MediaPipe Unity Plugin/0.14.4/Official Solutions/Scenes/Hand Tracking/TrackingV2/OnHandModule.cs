...
namespace Mediapipe.Unity.Sample.HandTracking
{
    public class OnHandModule : MonoBehaviour
    {
        ...
        [SerializeField] public GameObject menuCaller; // Button for show menu
        ...
        void Update()
        {
            if (mainTrackingModule.IsListActive)
            {
                if (mainTrackingModule._currentHandLandmarkLists.Count > 1)
                {
                    ...
                    MiddlePointer.gameObject.transform.localPosition = new Vector3(0 - mainTrackingModule._screen.GetComponent<RectTransform>().rect.GetPoint(mainTrackingModule._currentHandLandmarkLists[1].Landmark[0]).x, mainTrackingModule._screen.GetComponent<RectTransform>().rect.GetPoint(mainTrackingModule._currentHandLandmarkLists[1].Landmark[0]).y, mainTrackingModule._screen.GetComponent<RectTransform>().rect.GetPoint(mainTrackingModule._currentHandLandmarkLists[1].Landmark[0]).z);
