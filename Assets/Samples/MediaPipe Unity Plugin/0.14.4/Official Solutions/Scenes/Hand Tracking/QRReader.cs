...
namespace Mediapipe.Unity.Sample.HandTracking
{
    public class QRReader : MonoBehaviour
    {
        ...
        private IEnumerator ReadPixels()
        {
            ...
            texture2D.SetPixels32(request.GetData<Color32>().ToArray());
            texture2D.Apply();
            byte[] bytearr = texture2D.EncodeToJPG();
            ScreenShotInString = Convert.ToBase64String(bytearr);
        }
        private void Start()
        {
            QRTexture = new Texture2D(1280, 800, TextureFormat.RGBA32, false);
            IncomeImage = new Texture2D(1, 1);
            texture2D = new Texture2D(ScreenShotCamera.pixelWidth, ScreenShotCamera.pixelHeight, TextureFormat.ARGB32, false);
            renderTexture = new RenderTexture(ScreenShotCamera.pixelWidth, ScreenShotCamera.pixelHeight, 24);
